...
class BrokerConnConfig(DataClassBase):
    ...
    REDIS_DB_FILTER_AND_RPC_RESULT = (
        8  # 如果函数做任务参数过滤 或者使用rpc获取结果，使用这个db，因为这个db的键值对多，和redis消息队列db分开
    )
    ...
    TXT_FILE_PATH = (
        Path(__file__).parent / "txt_queues"
    )  # 不建议使用这个txt模拟消息队列中间件，本地持久化优先选择 PERSIST_QUQUE 中间件。
    ...
    CELERY_BROKER_URL = (
        "redis://127.0.0.1:6379/12"  # 使用celery作为中间件。funboost新增支持celery框架来运行函数,url内容就是celery的broker形式.
    )
...
class FunboostCommonConfig(DataClassBase):
    ...
    NB_LOG_FORMATER_INDEX_FOR_CONSUMER_AND_PUBLISHER = logging.Formatter(
        ...
        "%Y-%m-%d %H:%M:%S",
    )  # 这个是带task_id的日志模板,日志可以显示task_id,方便用户串联起来排查某一个任务消息的所有日志.
    ...
    KEEPALIVETIMETHREAD_LOG_LEVEL = (
        logging.INFO
    )  # funboost的作者发明的可缩小自适应线程池,用户对可变线程池的线程创建和销毁线程完全无兴趣,可以提高日志级别.
